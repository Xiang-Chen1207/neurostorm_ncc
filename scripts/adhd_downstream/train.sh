#!/bin/bash
# Fine-tuning NeuroSTORM on ADHD dataset for ADHD vs Control classification
# Usage: bash scripts/adhd_downstream/train.sh [batch_size]
...
project_name="adhd_ft_neurostorm_classification_epoch20"
...
python /home/chenx/code/neurostorm_ncc/main.py \
  ...
  --clf_head_version v1 \
  --dataset_name ADHD_NEW \
  --image_path /home/chenx/code/neurostorm_ncc/data/adhd \
...
# - Only the first file per subject is used, and only the first 20 frames
# - Labels (ADHD=1, Control=0) are extracted from adhd_dx_group.csv based on Subject ID
# - Subject ID is extracted from filename (e.g., "0010001_run-1_0000-0199_1.npz" -> "0010001")
