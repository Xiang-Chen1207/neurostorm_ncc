#!/bin/bash
# Fine-tuning NeuroSTORM on HCP dataset for sex classification
# Usage: bash scripts/hcp_downstream/train.sh [batch_size]
...
# - Only the first 20 frames from each .npz file are used
# - Labels (Female=0, Male=1) are extracted from hcp.csv based on subject ID
